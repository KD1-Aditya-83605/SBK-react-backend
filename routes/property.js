...
router.post('/update', upload.single('image'), (request, response) => {
  ...
  const query = `update property set categoryId= ?,title=?, details=?,address=?,contactNo=?,ownerName=?,isLakeView=?,isTV=?,isAC=?,isWifi=?,isMiniBar=?,isBreakfast=?,isParking=?,guests=?,bedrooms=?,beds=?,bathrooms=?,rent=?, profileImage=? where id=${id}`
  db.pool.execute(
    query,
    [
      1,
